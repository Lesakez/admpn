...
const profileSchema = Joi.object({
  profileId: Joi.string().min(1).max(255).allow('', null).messages({
    'string.empty': 'Profile ID не может быть пустым',
    'string.max': 'Profile ID не может быть длиннее 255 символов'
  }),
  profileName: Joi.string().min(1).max(255).required().messages({
...
const updateProfileSchema = Joi.object({
  profileId: Joi.string().min(1).max(255).allow('', null),
  profileName: Joi.string().min(1).max(255),
  folderId: Joi.string().max(255).allow('', null),
  folderName: Joi.string().max(255).allow('', null),
